...
class Spaceship(Base):
    ...
    speed: Mapped[float] = mapped_column(Integer(), nullable=True)
...
url = sqlalchemy.engine.url.URL.create(drivername="postgresql",
                                       username="postgres", password="3454325",
                                       database="pythonGRPC", host="localhost", port=5432)
...
def save_ship(grpc_spaceship) -> None:
    try:
        with Session(engine) as session:
